    ...
    try {
        ...
        if (req.method == "GET") {
            ...
            var [profiles] = await connection
                .promise()
                .query(`SELECT Users.username, Users.id, Users.email, Business.location, Business.logo FROM Users JOIN Business on Users.id = Business.user_id UNION ALL SELECT Users.username, Users.id, Users.email, Individual.location, Individual.profile_picture AS logo FROM Users JOIN Individual on Users.id = Individual.user_id;`)
            let temp = []
