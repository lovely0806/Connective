  ...
  try {
    ...
    if (req.method == "GET") {
      ...
      var [results, fields, err] = await connection
        .promise()
        .query(
          `SELECT Lists.*, Business.company_name AS username, Business.logo FROM Lists JOIN Business on Lists.creator = Business.user_id WHERE Lists.published = 1 UNION ALL SELECT Lists.*, Individual.name AS username, Individual.profile_picture AS logo FROM Lists JOIN Individual on Lists.creator = Individual.user_id WHERE Lists.published = 1;`
        );
    ...
    if (req.method == "PUT") {
      ...
      let [result] = await connection
        .promise()
        .execute(`SELECT creator FROM Lists WHERE id = ${id}`);
      if (!result.length) res.status(404).json({ success: false });
      if (user.id != result[0].creator)
        res.status(401).json({ success: false });
      [result, err] = await connection.promise().execute(`
                  UPDATE Lists SET title = '${title}', category = '${category}', description = '${description}', location = '${geo}', ${
        fileChanged && "preview_url =" + `'${previewUrl}',`
      } ${
        coverChanged && "cover_url =" + `'${coverUrl}',`
      } list_obtained = '${obtain}'
                    WHERE id = ${id}`);

      await connection
        .promise()
        .execute(`DELETE FROM Fields WHERE list_id = ${id}`);

      let fieldValues = [];
      fields.forEach((field) => {
        fieldValues.push([id, field.name, field.description]);
      });
