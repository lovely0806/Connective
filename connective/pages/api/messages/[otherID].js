...
import { ActivityFeed } from "../../../services/activity/activityFeed.ts";
...
export async function handler(req, res) {
  ...
  try {
    ...
    if (req.method == "POST") {
      const { text } = req.body;
      await ActivityFeed.Messages.handleMessage(user.id, otherID, text);
      ...
      var [results] = await connection
        ...
        .query(
          `INSERT INTO messages (` +
            ...
            `, ` +
            "`notified`" +
            `) VALUES ('${user.id}', '${otherID}', '${text}', '0', '0')`
        );
      res.status(200).json(results);
    }
  } catch (e) {
